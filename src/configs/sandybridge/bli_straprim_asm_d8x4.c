...
void bli_dstraprim_asm_8x4( dim_t k,
                            ...
                            auxinfo_t* data )
{
    ...
    //printf( "enter bli_dstraprim_asm_8x4\n" );
  ...
  //printf( "ldc = %lu\n", ldc );
  //printf( "cA_start: " );
  //printf( "%lf, %lf, %lf, %lf\n", cA[0], cA[ ldc + 0], cA[ ldc * 2 + 0], cA[ ldc * 3 + 0] );
  //printf( "%lf, %lf, %lf, %lf\n", cA[1], cA[ ldc + 1], cA[ ldc * 2 + 1], cA[ ldc * 3 + 1] );
  //printf( "%lf, %lf, %lf, %lf\n", cA[2], cA[ ldc + 2], cA[ ldc * 2 + 2], cA[ ldc * 3 + 2] );
  //printf( "%lf, %lf, %lf, %lf\n", cA[3], cA[ ldc + 3], cA[ ldc * 2 + 3], cA[ ldc * 3 + 3] );
  //printf( "%lf, %lf, %lf, %lf\n", cA[4], cA[ ldc + 4], cA[ ldc * 2 + 4], cA[ ldc * 3 + 4] );
  //printf( "%lf, %lf, %lf, %lf\n", cA[5], cA[ ldc + 5], cA[ ldc * 2 + 5], cA[ ldc * 3 + 5] );
  //printf( "%lf, %lf, %lf, %lf\n", cA[6], cA[ ldc + 6], cA[ ldc * 2 + 6], cA[ ldc * 3 + 6] );
  //printf( "%lf, %lf, %lf, %lf\n", cA[7], cA[ ldc + 7], cA[ ldc * 2 + 7], cA[ ldc * 3 + 7] );
  //printf( "cB_start: " );
  //printf( "%lf, %lf, %lf, %lf\n", cB[0], cB[ ldc + 0], cB[ ldc * 2 + 0], cB[ ldc * 3 + 0] );
  //printf( "%lf, %lf, %lf, %lf\n", cB[1], cB[ ldc + 1], cB[ ldc * 2 + 1], cB[ ldc * 3 + 1] );
  //printf( "%lf, %lf, %lf, %lf\n", cB[2], cB[ ldc + 2], cB[ ldc * 2 + 2], cB[ ldc * 3 + 2] );
  //printf( "%lf, %lf, %lf, %lf\n", cB[3], cB[ ldc + 3], cB[ ldc * 2 + 3], cB[ ldc * 3 + 3] );
  //printf( "%lf, %lf, %lf, %lf\n", cB[4], cB[ ldc + 4], cB[ ldc * 2 + 4], cB[ ldc * 3 + 4] );
  //printf( "%lf, %lf, %lf, %lf\n", cB[5], cB[ ldc + 5], cB[ ldc * 2 + 5], cB[ ldc * 3 + 5] );
  //printf( "%lf, %lf, %lf, %lf\n", cB[6], cB[ ldc + 6], cB[ ldc * 2 + 6], cB[ ldc * 3 + 6] );
  //printf( "%lf, %lf, %lf, %lf\n", cB[7], cB[ ldc + 7], cB[ ldc * 2 + 7], cB[ ldc * 3 + 7] );
	...
	"                                            \n\t"
	"                                            \n\t"
    "movq         %4, %%rax                      \n\t" // load address of alpha1
	"movq         %5, %%rbx                      \n\t" // load address of alpha2
	"vbroadcastsd    (%%rax), %%ymm7             \n\t" // load alpha1 and duplicate
	"vbroadcastsd    (%%rbx), %%ymm6             \n\t" // load alpha2 and duplicate
	"                                            \n\t"
    "movq         %6, %%rax                      \n\t" // load address of beta1
	"movq         %7, %%rbx                      \n\t" // load address of beta2
	"vbroadcastsd    (%%rax), %%ymm5             \n\t" // load beta1 and duplicate
	"vbroadcastsd    (%%rbx), %%ymm4             \n\t" // load beta2 and duplicate
	"                                            \n\t"
	"                                            \n\t"
	"                                            \n\t"
//	"jmp    .DDONE                               \n\t" // jump to end. For DEBUGging purpose
	"                                            \n\t"
	"                                            \n\t"
	"                                            \n\t"
...
//	"vmulpd            %%ymm0,  %%ymm5,  %%ymm0  \n\t" // scale by beta1
	...
	"vmulpd            %%ymm6,  %%ymm9,  %%ymm2  \n\t" // scale by alpha2, ymm2 = ymm6( alpha2 ) * ymm9( ab0_3:0 )
	...
	"vmulpd            %%ymm7,  %%ymm8,  %%ymm1  \n\t" // scale by alpha1, ymm1 = ymm7( alpha1 ) * ymm8( ab4_7:0 )
	"vaddpd            %%ymm1,  %%ymm0,  %%ymm1  \n\t" // ymm1 = ymm0 + ymm1
	...
	"vaddpd            %%ymm1,  %%ymm0,  %%ymm1  \n\t" // ymm1 = ymm0 + ymm1
	"vmulpd            %%ymm6,  %%ymm11, %%ymm2  \n\t" // scale by alpha2, ymm2 = ymm6( alpha2 ) * ymm11( ab0_3:1 )
	"vaddpd            %%ymm2,  %%ymm3,  %%ymm2  \n\t" // ymm2 = ymm3 + ymm2
	"vmovapd           %%ymm1,  0 * 32(%%rcx)    \n\t" // and store back to memory: cA.
	"vmovapd           %%ymm2,  0 * 32(%%rdx)    \n\t" // and store back to memory: cB.
	"                                            \n\t"
	"vmovapd    1 * 32(%%rcx),  %%ymm0           \n\t" // ymm0 = cA( 4:7, 1 )
	"vmovapd    1 * 32(%%rdx),  %%ymm3           \n\t" // ymm3 = cB( 4:7, 1 )
	"vmulpd            %%ymm7,  %%ymm10, %%ymm1  \n\t" // scale by alpha1, ymm1 = ymm7( alpha1 ) * ymm10( ab4_7:1 )
	"vaddpd            %%ymm1,  %%ymm0,  %%ymm1  \n\t" // ymm1 = ymm0 + ymm1
	...
	"vmovapd    0 * 32(%%rcx),  %%ymm0           \n\t" // ymm0 = cA( 0:3, 2 )
	"vmovapd    0 * 32(%%rdx),  %%ymm3           \n\t" // ymm3 = cB( 0:3, 2 )
	"vmulpd            %%ymm7,  %%ymm13, %%ymm1  \n\t" // scale by alpha1, ymm1 = ymm7( alpha1 ) * ymm13( ab0_3:2 )
	"vaddpd            %%ymm1,  %%ymm0,  %%ymm1  \n\t" // ymm1 = ymm0 + ymm1
	"vmulpd            %%ymm6,  %%ymm13, %%ymm2  \n\t" // scale by alpha2, ymm2 = ymm6( alpha2 ) * ymm13( ab0_3:2 )
	"vaddpd            %%ymm2,  %%ymm3,  %%ymm2  \n\t" // ymm2 = ymm3 + ymm2
	"vmovapd           %%ymm1,  0 * 32(%%rcx)    \n\t" // and store back to memory: cA.
	"vmovapd           %%ymm2,  0 * 32(%%rdx)    \n\t" // and store back to memory: cB.
	"                                            \n\t"
	"vmovapd    1 * 32(%%rcx),  %%ymm0           \n\t" // ymm0 = cA( 4:7, 2 )
	"vmovapd    1 * 32(%%rdx),  %%ymm3           \n\t" // ymm3 = cB( 4:7, 2 )
	"vmulpd            %%ymm7,  %%ymm12, %%ymm1  \n\t" // scale by alpha1, ymm1 = ymm7( alpha1 ) * ymm12( ab4_7:2 )
	"vaddpd            %%ymm1,  %%ymm0,  %%ymm1  \n\t" // ymm1 = ymm0 + ymm1
	"vmulpd            %%ymm6,  %%ymm12, %%ymm2  \n\t" // scale by alpha2, ymm2 = ymm6( alpha2 ) * ymm12( ab4_7:2 )
	"vaddpd            %%ymm2,  %%ymm3,  %%ymm2  \n\t" // ymm2 = ymm3 + ymm2
	"vmovapd           %%ymm1,  1 * 32(%%rcx)    \n\t" // and store back to memory: cA.
	"vmovapd           %%ymm2,  1 * 32(%%rdx)    \n\t" // and store back to memory: cB.
	"                                            \n\t"
	...
	"vmovapd    0 * 32(%%rcx),  %%ymm0           \n\t" // ymm0 = cA( 0:3, 3 )
	"vmovapd    0 * 32(%%rdx),  %%ymm3           \n\t" // ymm3 = cB( 0:3, 3 )
	"vmulpd            %%ymm7,  %%ymm15, %%ymm1  \n\t" // scale by alpha1, ymm1 = ymm7( alpha1 ) * ymm15( ab0_3:3 )
	"vaddpd            %%ymm1,  %%ymm0,  %%ymm1  \n\t" // ymm1 = ymm0 + ymm1
	"vmulpd            %%ymm6,  %%ymm15, %%ymm2  \n\t" // scale by alpha2, ymm2 = ymm6( alpha2 ) * ymm15( ab0_3:3 )
	"vaddpd            %%ymm2,  %%ymm3,  %%ymm2  \n\t" // ymm2 = ymm3 + ymm2
	"vmovapd           %%ymm1,  0 * 32(%%rcx)    \n\t" // and store back to memory: cA.
	"vmovapd           %%ymm2,  0 * 32(%%rdx)    \n\t" // and store back to memory: cB.
	"                                            \n\t"
	"vmovapd    1 * 32(%%rcx),  %%ymm0           \n\t" // ymm0 = cA( 4:7, 3 )
	"vmovapd    1 * 32(%%rdx),  %%ymm3           \n\t" // ymm3 = cB( 4:7, 3 )
	"vmulpd            %%ymm7,  %%ymm14, %%ymm1  \n\t" // scale by alpha1, ymm1 = ymm7( alpha1 ) * ymm14( ab4_7:3 )
	"vaddpd            %%ymm1,  %%ymm0,  %%ymm1  \n\t" // ymm1 = ymm0 + ymm1
	"vmulpd            %%ymm6,  %%ymm14, %%ymm2  \n\t" // scale by alpha2, ymm2 = ymm6( alpha2 ) * ymm14( ab4_7:3 )
	"vaddpd            %%ymm2,  %%ymm3,  %%ymm2  \n\t" // ymm2 = ymm3 + ymm2
	"vmovapd           %%ymm1,  1 * 32(%%rcx)    \n\t" // and store back to memory: cA.
	"vmovapd           %%ymm2,  1 * 32(%%rdx)    \n\t" // and store back to memory: cB.
	"                                            \n\t"
...
//	"vmovapd           %%ymm14, 32(%%rcx)         \n\t" // C_c( 7, 0:3 ) = ymm14
	...
	"                                            \n\t"
	"vmulpd            %%ymm7,  %%ymm11, %%ymm1  \n\t" // scale by alpha1, ymm1 = ymm7( alpha1 ) * ymm11( ab0_3:1 )
	"vmulpd            %%ymm6,  %%ymm11, %%ymm2  \n\t" // scale by alpha2, ymm2 = ymm6( alpha2 ) * ymm10( ab0_3:1 )
	"vmovapd           %%ymm1,  0 * 32(%%rcx)    \n\t" // and store back to memory: cA.
	"vmovapd           %%ymm2,  0 * 32(%%rdx)    \n\t" // and store back to memory: cB.
	"                                            \n\t"
	"vmulpd            %%ymm7,  %%ymm10, %%ymm1  \n\t" // scale by alpha1, ymm1 = ymm7( alpha1 ) * ymm10( ab4_7:1 )
