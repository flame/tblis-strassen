...
{
    //std::cout << "Enter random seq generator" << std::endl;
    TBLIS_ASSERT(n >  0);
    TBLIS_ASSERT(p >= 1);
    TBLIS_ASSERT(mn.size() == n);
    ...
    for (unsigned i = 0;i < n;i++) s[i] = std::exp(s[i]);
    //for (unsigned i = 0;i < n;i++) s[i] = (s[i]/2)*2;
...
random_product_constrained_sequence(unsigned n, T p)
{
    TBLIS_ASSERT(n > 0);
    //p = p / 2;
    std::vector<T> s = random_product_constrained_sequence(n, p, std::vector<T>(n, 1.0));
    //s[ s.size() - 1 ] *= 2;
    return s;
...
random_product_constrained_sequence(unsigned n, T p, const std::vector<T>& mn)
{
    ...
    for (unsigned i = 0;i < n;i++)
    {
        ...
        //mnd[i] = std::max(T(2), mn[i]);
    }

    std::vector<double> sd = random_product_constrained_sequence<double>(n, p/adjust, mnd);
