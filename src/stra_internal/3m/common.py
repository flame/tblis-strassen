...
def writeSubmat( myfile, mat_name, dim1, dim2, split1, split2, src_mat_id ): 
    ...
    for ii in range( split1 ):
        for jj in range( split2 ):
            ...
            write_line( myfile, 1, "stra_acquire_mpart( {0}, {1}, rs_{2}, cs_{2}, {5}, {6}, {7}, {8}, {2}{3}, &{2}{3}{4} );".format( dim1, dim2, mat_name, str(src_mat_id), '_'+str(ii * split2 + jj), split1, split2, ii, jj ) )
...
def writePartition( myfile, dims, level=1 ):
    ...
    write_line( myfile, 1, "len_type ms, ks, ns;" )
    write_line( myfile, 1, "len_type md, kd, nd;" )
    write_line( myfile, 1, "len_type mr, kr, nr;" )
