#ifndef _TBLIS_STRA_NODES_PARTM_HPP_
#define _TBLIS_STRA_NODES_PARTM_HPP_
...
struct stra_partition
{
                    ...
                    T alpha, MatrixA& A, MatrixB& B, T beta, MatrixC& C)
    {
        ...
        //std::cout << "Enter stra_partition" << std::endl;
        ...
        ////printf("A before: %p %ld %ld %ld %ld\n", A.data(0), A.length(0), A.length(1), A.stride(0), A.stride(1));
        ////printf("B before: %p %ld %ld %ld %ld\n", B.data(0), B.length(0), B.length(1), B.stride(0), B.stride(1));
        ////printf("C before: %p %ld %ld %ld %ld\n", C.data(0), C.length(0), C.length(1), C.stride(0), C.stride(1));
        //printf("A before: %ld %ld %ld %ld\n", A.length(0), A.length(1), A.stride(0), A.stride(1));
        //printf("B before: %ld %ld %ld %ld\n", B.length(0), B.length(1), B.stride(0), B.stride(1));
        //printf("C before: %ld %ld %ld %ld\n", C.length(0), C.length(1), C.stride(0), C.stride(1));
        ...
        //printf("A after: %ld %ld %ld %ld\n", A.length(0), A.length(1), A.stride(0), A.stride(1));
        //printf("B after: %ld %ld %ld %ld\n", B.length(0), B.length(1), B.stride(0), B.stride(1));
        //printf("C after: %ld %ld %ld %ld\n", C.length(0), C.length(1), C.stride(0), C.stride(1));
...
using stra_partition_gemm_mc = stra_partition<matrix_constants::DIM_M, &config::gemm_mc, Child>;

template <typename Child>
using stra_partition_gemm_mr = stra_partition<matrix_constants::DIM_M, &config::gemm_mr, Child>;

template <typename Child>
using stra_partition_gemm_nc = stra_partition<matrix_constants::DIM_N, &config::gemm_nc, Child>;

template <typename Child>
using stra_partition_gemm_nr = stra_partition<matrix_constants::DIM_N, &config::gemm_nr, Child>;

template <typename Child>
using stra_partition_gemm_kc = stra_partition<matrix_constants::DIM_K, &config::gemm_kc, Child>;
