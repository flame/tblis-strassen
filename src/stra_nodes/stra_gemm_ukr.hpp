...
void stra_accum_utile_list(len_type m, len_type n,
                 const T* TBLIS_RESTRICT p_ab, stride_type rs_ab, stride_type cs_ab,
                 T beta,
                 T** TBLIS_RESTRICT p_c_list, const T* coeff_list,
                 ...
                 const stride_type** cs_c)
{
    ...
    //for (unsigned idx = 0; idx < N; idx++) {
    //    std::cout << "before: C[" << idx << "]:" << std::endl;
    //    for (len_type i = 0;i < m;i++)
    //    {
    //        for (len_type j = 0;j < n;j++)
    //        {
...
void stra_accum_utile_list(len_type m, len_type n,
                 ...
                 T beta,
                 T** TBLIS_RESTRICT p_c_list, const T* coeff_list,
                 stride_type rs_c, stride_type cs_c)
{
    //std::cout << "beta:" << beta << std::endl;
    if (beta == T(0))
    {
...
template <typename T>
void stra_accum_utile(len_type m, len_type n,
                 const T* TBLIS_RESTRICT p_ab, stride_type rs_ab, stride_type cs_ab,
...
struct stra_gemm_micro_kernel
{
                    ...
                    T  beta, matrix_view<T>& C) const
    {
        ...
        else
        {
            ...
            stra_accum_utile(m, n, p_ab, rs_ab, cs_ab,
                        beta, p_c, rs_c, cs_c);
        }
    }
                    ...
                    T  beta, stra_matrix_view<T,N>& C) const
    {
        ...
        //stra_accum_utile_list(m, n, p_ab, rs_ab, cs_ab,
        ...
        else
        {
            ...
            if (N == 1) {
                stra_accum_utile(m, n, p_ab, rs_ab, cs_ab,
                        beta, p_c_list[0], rs_c, cs_c);
            //} else if ( N == 2 ) {
            //    stra_accum_utile_list_two(m, n, p_ab, rs_ab, cs_ab,
            //            beta, N, p_c_list, coeff_list, rs_c, cs_c);
            //} else if ( N == 4 ) {
            //    stra_accum_utile_list_four(m, n, p_ab, rs_ab, cs_ab,
            //            beta, N, p_c_list, coeff_list, rs_c, cs_c);
            } else {
                //std::cout << "stra_accum_utile_list: N is not 1,2,4, N: " << N << std::endl;
                stra_accum_utile_list<N>(m, n, p_ab, rs_ab, cs_ab,
                        beta, p_c_list, coeff_list, rs_c, cs_c);
            }
                    ...
                    T  beta, stra_block_scatter_matrix<T,N>& C) const
    {
        ...
        for (unsigned idx = 0; idx < N; idx++) {
            if ( C.stride(idx, 0) == 0 || C.stride(idx, 0) != C.stride(0, 0) ) {
                is_all_rs_a_nonzero_same = false;
            }
        }
        bool is_all_cs_a_nonzero_same = true;
        for (unsigned idx = 0; idx < N; idx++) {
            if ( C.stride(idx, 1) == 0 || C.stride(idx, 1) != C.stride(0, 1) ) {
        ...
        for (unsigned idx = 0; idx < N; idx++)
        {
            //p_c_list[idx] = C.raw_data();
            //p_c_list[idx] = c_list[idx];
            //p_c_list[idx] = c_list[idx] + (C.stride(idx, 0) == 0 ? 0 : *(C.scatter(idx, 0))) + (C.stride(idx, 1) == 0 ? 0 : *(C.scatter(idx, 1)));

            //p_c_list[idx] = C.data(idx);

            p_c_list[idx] = C.raw_data() + ( is_all_rs_a_nonzero_same == 0 ? 0 : *(C.scatter(idx, 0))) + ( is_all_cs_a_nonzero_same == 0 ? 0 : *(C.scatter(idx, 1)));
        }


        //std::cout << "p_c_list[0][0]: " << p_c_list[0][0] << std::endl;
        //if ( N == 2 ) {
        //    std::cout << "p_c_list[1][0]: " << p_c_list[1][0] << std::endl;
        //}
        ...
        //std::cout << "rs_c[0]:" << C.stride(0,0) << "; cs_c[0]:" << C.stride(0,1);
        //std::cout << "; rs_c[1]:" << C.stride(1,0) << "; cs_c[1]:" << C.stride(1,1) << std::endl;
        ...
        //std::cout << "rscat_c[0][0]: " << rscat_c[0][0]  << "; cscat_c[0][0]: " << cscat_c[0][0] << "rscat_c[0][1]: " << rscat_c[0][1]  << "; cscat_c[0][1]: " << cscat_c[0][1] << std::endl;
        //std::cout << "rscat_c[1][0]: " << rscat_c[1][0]  << "; cscat_c[1][0]: " << cscat_c[1][0] << "rscat_c[1][1]: " << rscat_c[1][1]  << "; cscat_c[1][1]: " << cscat_c[1][1] << std::endl;
        ...
        //std::cout << "rscat_c:" << std::endl;
        //for (int idx=0; idx < N; idx++)
        //{
        //    std::cout << "idx" << idx << std::endl;
        //    for (int i = 0; i < m; i++)
        //    {
        //        std::cout << rscat_c[idx][i] << " ";
        //    }
        //    std::cout << std::endl;
        //}
        //std::cout << "cscat_c:" << std::endl;
        //for (int idx=0; idx < N; idx++)
        //{
        //    std::cout << "idx" << idx << std::endl;
        //    for (int i = 0; i < n; i++)
        //    {
        //        std::cout << cscat_c[idx][i] << " ";
        //    }
        //    std::cout << std::endl;
        //}
        ...
        //Version 1
        //if (m == MR && n == NR && rs_c != 0 && cs_c != 0)
        if (m == MR && n == NR && is_all_rs_a_nonzero_same && is_all_cs_a_nonzero_same)
        {
        ...
        else
        {
            ...
            //if (rs_c == 0 && cs_c == 0)
            if (!is_all_rs_a_nonzero_same && !is_all_cs_a_nonzero_same)
            {
                ...
                if (N == 1) {
                    stra_accum_utile(m, n, p_ab, rs_ab, cs_ab,
                            beta, p_c_list[0], rscat_c[0], cscat_c[0]);
                //} else if ( N == 2 ) {
                //    stra_accum_utile_list_two(m, n, p_ab, rs_ab, cs_ab,
                //            beta, N, p_c_list, coeff_list, rscat_c, cscat_c);
                //} else if ( N == 4 ) {
                ...
                //            beta, N, p_c_list, coeff_list, rscat_c, cscat_c);
                    ...
                    //std::cout << "N is not 1,2,4" << std::endl;
                    stra_accum_utile_list<N>(m, n, p_ab, rs_ab, cs_ab,
                            beta, p_c_list, coeff_list, rscat_c, cscat_c);
                }

            }
            //else if (rs_c == 0)
            else if (!is_all_rs_a_nonzero_same)
            {
                //std::cout << "rs_c == 0 && cs_c != 0" << std::endl;
                //stra_accum_utile_list(m, n, p_ab, rs_ab, cs_ab,
                ...
                if (N == 1) {
                    stra_accum_utile(m, n, p_ab, rs_ab, cs_ab,
                            beta, p_c_list[0], rscat_c[0], cs_c);
                //} else if ( N == 2 ) {
                //    stra_accum_utile_list_two(m, n, p_ab, rs_ab, cs_ab,
                //            beta, N, p_c_list, coeff_list, rscat_c, cs_c);
                //} else if ( N == 4 ) {
                ...
                //            beta, N, p_c_list, coeff_list, rscat_c, cs_c);
                    ...
                    //std::cout << "N is not 1,2,4" << std::endl;
                    stra_accum_utile_list<N>(m, n, p_ab, rs_ab, cs_ab,
                            beta, p_c_list, coeff_list, rscat_c, cs_c);
                }

            }
            //else if (cs_c == 0)
            else if (!is_all_cs_a_nonzero_same)
            {
                //std::cout << "rs_c != 0 && cs_c == 0" << std::endl;
                //stra_accum_utile_list(m, n, p_ab, rs_ab, cs_ab,
                ...
                if (N == 1) {
                    stra_accum_utile(m, n, p_ab, rs_ab, cs_ab,
                            beta, p_c_list[0], rs_c, cscat_c[0]);
                //} else if ( N == 2 ) {
                //    stra_accum_utile_list_two(m, n, p_ab, rs_ab, cs_ab,
                //            beta, N, p_c_list, coeff_list, rs_c, cscat_c);
                //} else if ( N == 4 ) {
                ...
                //            beta, N, p_c_list, coeff_list, rs_c, cscat_c);
                    ...
                    //std::cout << "N is not 1,2,4" << std::endl;
                    stra_accum_utile_list<N>(m, n, p_ab, rs_ab, cs_ab,
                            beta, p_c_list, coeff_list, rs_c, cscat_c);
                }
            }
            else
            {
        ...
        //std::cout << "leave micro-kernel" << std::endl;
        ...
        //std::cout << "p_ab:" << std::endl;
        //for (int i=0; i < m; i++)
        //{
        //    for (int j = 0; j < n; j++)
        //    {
        //        std::cout << p_ab[i*rs_ab+j*cs_ab] << " ";
        //    }
        //    std::cout << std::endl;
        //}
